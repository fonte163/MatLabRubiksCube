...
function rotate_view(~, ~)
	global elon;
	global azim;
	view( get(azim, 'Value')*180-90,get(elon, 'Value')*360);
end
...
function cmenu=generate_single_ui_menu(i, j)
	...
	uimenu(cmenu(i,j), 'Label', 'white', 'Callback', @change_color, 'UserData', [i j]);
	uimenu(cmenu(i,j), 'Label', 'red', 'Callback', @change_color, 'UserData', [i j]);
	uimenu(cmenu(i,j), 'Label', 'blue', 'Callback', @change_color, 'UserData', [i j]);
	uimenu(cmenu(i,j), 'Label', 'orange', 'Callback', @change_color, 'UserData', [i j]);
	uimenu(cmenu(i,j), 'Label', 'green', 'Callback', @change_color, 'UserData', [i j]);
	uimenu(cmenu(i,j), 'Label', 'yellow', 'Callback', @change_color, 'UserData', [i j]);
end
